...
resource "google_monitoring_alert_policy" "gateway_5xx" {
  ...
  lifecycle {
    precondition {
      condition     = var.deployment_environment != "production" || length(var.alert_notification_channels) > 0
      error_message = "Production deployments must configure at least one Cloud Monitoring notification channel."
    }
  }
...
resource "google_monitoring_alert_policy" "billing_api_5xx" {
  display_name          = "${var.billing_api_service_name} 5xx responses"
  combiner              = "OR"
  enabled               = true
  notification_channels = var.alert_notification_channels
  ...
  documentation {
    mime_type = "text/markdown"
    content   = "The Billing API returned 5xx responses. Check Stripe webhook delivery attempts and the internal cancellation reconciliation logs."
  }

  conditions {
    display_name = "Billing API 5xx count above zero"

    condition_threshold {
      filter          = <<-EOT
metric.type="run.googleapis.com/request_count"
resource.type="cloud_run_revision"
resource.label."service_name"="${var.billing_api_service_name}"
metric.label."response_code_class"="5xx"
EOT
      comparison      = "COMPARISON_GT"
      threshold_value = 0
  ...
  user_labels = {
    ...
    service   = var.billing_api_service_name
  }
